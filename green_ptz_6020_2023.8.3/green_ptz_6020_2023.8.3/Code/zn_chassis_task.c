...
int yaw_motor_mid=-3390;
int pitch_motor_mid=-3600;
...
float x_angle=0,y_angle=0;//-105  -62
float x_p=1,y_p=1;
int flag,err_flag;
int sdfghj=200;
...
//  if(speed_a1<=-550){speed_a1=-550;}
//  if(speed_a1>=550){speed_a1=550;}
//  if(speed_a2>=650){speed_a2=650;}
//  if(speed_a2<=-650){speed_a2=-650;}
  ...
  if(err_x<=3&&err_x>=-3&&err_y<=3&&err_y>=-3&&err_flag==1){TIM1_SetPluse(sdfghj,sdfghj,sdfghj,sdfghj);}
  else {TIM1_SetPluse(0,0,0,0);}
...
float led_Kp=0.7,led_Ki=0.05,led_Kd=0;
...
float led_Kp_one=0.8,led_Ki_one=0.001,led_Kd_one=0;
